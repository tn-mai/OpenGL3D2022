...
プログラムを簡単にするため、`GameObject`クラスには、スプライトに関するパラメータをメンバ変数として組み込んでいます。一部のゲームエンジンでは、これらはコンポーネントとして作成されます。

しかし、画像の表示には固有の処理が非常に多く、描画専用のコンポーネントとその管理システムまで作る必要があります。ちょっと大変なので、今回はゲームオブジェクトに組み込むことを選びました。
...
+{
+  for (auto& e : componentList) {
+    e->OnTookDamage(*this, other, damage);
+  }
+}
+
+/**
...
`RemoveGameObject`関数は、死亡状態のゲームオブジェクトを配列から削除します。ここでは単に削除するだけでなく、削除するオブジェクトの`OnDestroy`関数を呼び出す必要があります。

そのために、`remove_if`の代わりに`partition`(パーティション)という関数を使っています。

<p><code class="tnmai_code"><strong>【書式】</strong><br>
iterator partition(調査範囲の先頭, 調査範囲の終端, 振り分け条件)
</code></p>

`partition`は、指定された範囲にある要素をすべて調べ、振り分け条件を「満たさない」要素を、範囲の前側に集めます。それ以外の削除してよい要素は後ろ側に移動されます。

`remove_if`は「一部の要素が削除される」場合がありますが、`partition`は「すべての要素が維持」され、何も削除されません。

振り分け中に削除されてしまうと、そのオブジェクトの`OnDestroy`関数を呼び出すことができません。そのため、`remove_if`ではなく`partition`を使うことにしました。

振り分けが終わったら、死んでいるオブジェクトを`deadList`(デッド・リスト)配列に移動します。配列から別の配列へ範囲を移動させるには、`move_iterator`(ムーブ・イテレータ)を使います。

`make_move_iterator`(メイク・ムーブ・イテレータ)は、通常のイテレータから`move_iterator`を作成する関数です。
...
まず優先順位を決めましょう。プロジェクトの`Src`フォルダに`SpritePriority.h`(スプライト・プライオリティ・エイチ)という名前のヘッダファイルを追加してください。
...
+* @file DamageSource.h
+*/
+#ifndef DAMAGESOURCE_H_INCLUDED
+#define DAMAGESOURCE_H_INCLUDED
+#include "../Component.h"
+#include "../GameObject.h"
+#include <string>
+
...
ダメージを受けるためには、`OnTookDamage`(オン・トゥック・ダメージ)をオーバーライドしたコンポーネントを作成します。
...
`Health`コンポーネントは、`OnTookDamage`(オン・トゥック・ダメージ)が実行されると`health`を減らします。そして、`health`が0以下になったら、割り当てられているゲームオブジェクトを死亡させます。
