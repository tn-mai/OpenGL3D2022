...
また、2Dタイルマップの大きさは17x110タイルなので、3Dマップの範囲は170x1100mとなります。というわけで、今回作成するレベルの定義は次のように決まりました。
...
X軸方向のマップサイズは170なので、`Ground`ゲームオブジェクトの`Scale`を(170, 170, 1)に設定してください。次に、`Position`を(85, 0, 85)に設定してください。これで1枚目の地面を敷くことができました。

しかし、マップは縦長なので、平面が1枚ではまったく足りません。Z軸方向のマップサイズは1100なので。マップ全体を埋めるには合計7枚の`Ground`ゲームオブジェクトが必要です。

`Ground`ゲームオブジェクトを選択し、`Ctrl+D`キーを押してコピーしてください。すると
`Ground (1)`のようなゲームオブジェクトが追加されます。

次に、このゲームオブジェクトの`Position`を(85, 0, 255)に設定してください。<br>
これで2枚目の地面を敷くことができました。

<pre class="tnmai_assignment">
<strong>【課題07】</strong>
<code>Ground</code>ゲームオブジェクトのコピーをあと5枚作成し、Z方向に隙間なく並べなさい。
</pre>
...
背景モデルの拡大率に`3.2f`を指定しているのは、縮尺を「3.2ピクセル=1メートル」と決めたからです。これによって、配置データのサイズをゲーム内のサイズと一致させています。

プログラムが書けたらビルドして実行してください。背景が表示されていたら成功です。

<p align="center">
<img src="images/11_result_6.jpg" width="45%" />
</p>
...
背景をスクロールさせると、自機や敵が建物に遮られて見づらいことに気づくと思います。これは、背景の座標が自機や敵と近すぎるからです。そこで、背景を少し遠くに配置します。<br>
3Dモデル配置ファイルを読み込むプログラムを、次のように変更してください。
...
`Destroyed City FREE`アセットの建物の高さは約100mですが、少しだけ地面に埋まるように配置したため、実際の地面から高さは約80mです。縮尺(3.2ピクセル=1メートル)を考慮すると、OpenGLにおける高さは256になります。

そのため、背景をZ軸方向に-300ほど移動させれば、自機や敵と重ならなくなるはずです。

プログラムが書けたらビルドして実行してください。背景が遠くなり、自機や敵が建物の上に表示されていたら成功です。

<p align="center">
<img src="images/11_result_7.jpg" width="45%" />
</p>

自機や敵が建物に重なることはなくなりましたが、背景の外側が見えてしまっています。3Dではカメラの視野角を考える必要があるのに、なにも対策をしていないからです。

背景をカメラから遠くに置いた状態で、背景がちょうど画面全体に表示されるようにするには、背景モデルに「原点からの距離に応じた拡大率」を乗算します。
...
```diff
     ...
     // 座標の拡大縮小と平行移動を適用
+    const float cameraScale = 1 - translate.z / cameraObject->z;
-    t = t * (scale * cameraScale) + translate;
-    s *= (scale * cameraScale);
+    t = t * (scale * cameraScale) + translate;
+    s *= (scale * cameraScale);

+    // 原点からの距離に応じて左下座標を補正
+    const vec2 zoomOffset(GetViewSize() * (cameraScale - 1) * 0.5f);
+    t.x -= zoomOffset.x;
+    t.y -= zoomOffset.y;
...
プログラムが書けたらビルドして実行してください。背景が画面全体に表示されていたら成功です。

<p align="center">
<img src="images/11_result_8.jpg" width="45%" />
</p>

うまくいっているように見えますが、よく見ると一部の建物の天井付近が、自機や敵がより手前に表示されています。原点からの距離に応じた拡大率を追加したことで、建物の高さも拡大されているからです。

現在はカメラのZ座標を500としていますが、この値では視野角が広すぎるため、「背景を画面いっぱいに表示する」と「自機や敵を建物の上に表示する」の両方を満たすことができません。

そこで、カメラをより遠くに配置して、視野角を<ruby>狭<rt>せば</rt></ruby>めます。以下の図は、カメラのZ座標を1000、背景のZ座標を-400にしたときの視野を示しています。

<p align="center">
<img src="images/11_view_range.png" width="33%" />
</p>

自機や敵の3Dモデルは`Z=0`のラインに配置されます。<br>
X軸方向の表示範囲は`17タイルx32ピクセル=544`なので、半径は272となります。

`Z=-400`のラインは背景モデルを配置する位置です。図形の相似から、サイズを約1.4倍すれば、`Z=0`に配置したときと同じ大きさに見えることが分かります。

この設定では、表示上の建物の最大の高さは`80*3.2*1.4=358.4`となります。この値は400より小さいため、建物の天井が自機や敵より低い位置に表示されるはずです。

それでは、カメラの設定を変更しましょう。`MainGameScene.cpp`を開き、`Initialize`メンバ関数にある「カメラパラメータを設定」するプログラムを、次のように変更してください。
...
また、現在のカメラのニア平面は1、ファー平面は1000になっています。カメラを遠くに配置したことと、背景を奥にずらしたことで、3Dモデルがファー平面より遠くに配置されるため、画面に映りません。

3Dモデルが表示されるようにするには、ニアとファーの値を変更します。アスペクト比を計算するプログラムの下に、次のプログラムを追加してください。

```diff
   // アスペクト比を計算
   cameraComponent->aspect = viewSize.y / viewSize.x;
+
+  // 3Dモデルの配置座標を、ニア平面とファー平面のあいだに入れる
+  cameraComponent->near = 500;
...
プログラムが書けたらビルドして実行してください。背景モデルが画面全体に表示され、なおかつ自機や敵が、常に建物の天井より上に表示されていたら成功です。
...
ところが、現実世界に目を向けてみると、太陽に裏を向けている部分でも、真っ黒にはなりません。なぜなら、裏面にも周囲の物体で反射したり大気で散乱した光が届いているからです。

このような、間接的に届く光の計算を「大域照明(グローバル・イルミネーション)」といいます。グローバルイルミネーションを<ruby>模倣<rt>もほう</rt></ruby>できれば、現実と同様の明るさを再現できます。
