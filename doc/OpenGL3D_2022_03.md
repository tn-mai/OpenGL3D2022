...
* `gl_InstanceID`を使って、インスタンスごとに位置やテクスチャ座標を変えることができる。
...
OpenGLで図形の位置を変更する方法は何種類もあり、時と場合によって適切な方法は異なります。一番簡単なのはユニフォーム変数を使うことです。

ちょっと実験してみましょう。`standard_2D.vert`を開き、次のプログラムを追加してください。
...
```diff
     ...
     // 図形を描画
+   glProgramUniform3f(progStandard2D, 1, 0.0f, 0.0f, 0.0f);
    glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, 0, 1);
+
+   glProgramUniform3f(progStandard2D, 1, -0.5f, 0.0f, 0.0f);
+   glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, 0, 1);
...
<strong>【課題01】</strong>
プログラムを追加して、画面に3つめの画像を表示しなさい。座標はどこでも構いません。
</pre>
...
+  std::mt19937 rg(rd()); // 疑似乱数を生成するオブジェクト(速い)
...
-   glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, 0, 1);
-
-   glProgramUniform3f(progStandard2D, 1, 0.0f, 0.5f, 0.0f);
-   glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, 0, 1);
+   for (const Sprite& e : spriteList) {
+     glProgramUniform3f(progStandard2D, 1, e.x, e.y, e.z);
+     glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, 0, 1);
+   }
...
「クリップ座標系」は、これまで定義してきた座標で使われている座標系です。クリップ座標系は

>画面に写る範囲を、X軸=`-1～+1`, Y軸=`-1～+1`, Z軸=`-1～+1`とする座標系

と定義されます。これに対し、「画面座標系」は

>画面に写る範囲を、X軸=`0～画面の幅`, Y軸=`0～画面の高さ`, Z軸=`-1～+1`とする座標系

と定義されます。`screenToClip`(スクリーン・トゥ・クリップ)変数には、画面座標系からクリップ座標系へと変換するためのパラメータを設定します。
...
```diff
     ...
     glProgramUniform1f(progStandard2D, 0, timer);
+
+    // ユニフォーム変数に、座標変換用パラメータをコピーする
+    int w, h;
+    glfwGetWindowSize(window, &w, &h);
+    glProgramUniform4f(progStandard2D, 2,
+      2.0f / (float)w, 2.0f / (float)h, -1, -1);
...
何も表示されない理由は、既存の座標がクリップ座標系で定義されているからです。2x2の大きさを想定していたため、画面座標系では値が小さすぎるのです。

そこで、図形の座標を画面座標系で表示できるサイズに変更します。`vertexData`配列を次のように変更してください。
...
プログラムが書けたらビルドして実行してください。次のように、画面いっぱいにたくさんの画像が表示されていたら成功です。

タイトルバーに表示されるFPSが、`for`ループで描画していたときと比較して、1.5倍くらいの数値になっていると思います。
...
それでは、`SSBO`を作成しましょう。スプライト座標を設定するプログラムの下に、次のプログラムを追加してください。
...
+
+  // スプライト用のSSBOを作成
+  const size_t spriteSsboSize = 
+    ((maxSpriteCount * sizeof(Sprite) + 255) / 256) * 256;
+  GLuint ssboSprite;
...
また、後述する「マッピング」という機能を使う場合、確保するバイト数を`256`の倍数にする必要があります。この制限を満たすバイト数は、

>`255`を足し、`256`で割り、`256`を掛ける。

という計算によって求められます。

さて、`SSBO`はバッファオブジェクトなので、`VBO`や`IBO`と同様に`glCreateBuffers`と`glNamedBufferStorage`関数を使って作成します。ただ、今回はいくつかのフラグを指定して作成します。使用したフラグは次のとおりです。

| フラグ名 | 効果 |
|:--------:|:-----|
| <ruby>GL_MAP_PERSISTENT_BIT<rt>ジーエル・マップ・パーシステント・ビット</rt></ruby> | 「マッピング」を解除しないタイプのバッファを作るように指示する |
| <ruby>GL_MAP_COHERENT_BIT<rt>ジーエル・マップ・コヒーレント・ビット</rt></ruby> | GPUが読み出し中にCPUが書き込んでも、GPUの描画に影響しないタイプのバッファを作るように指示する |
| <ruby>GL_MAP_WRITE_BIT<rt>ジーエル・マップ・ライト・ビット</rt></ruby> | CPUから書き込めるタイプのバッファを作るように指示する |
...
+  Sprite* pSpriteSsbo = (Sprite*)glMapNamedBuffer(ssboSprite, GL_WRITE_ONLY);
...
`glMapBuffer`関数は、「CPUから書き込めるアドレス」を返します。
...
+
+    // スプライトデータをSSBOにコピー
+    Sprite* p = pSpriteSsbo + spriteSsboIndex * maxSpriteCount;
+    std::copy(spriteList.begin(), spriteList.end(), p);

     // バックバッファをクリア
     glClearColor(0.3f, 0.6f, 0.9f, 1.0f);
```

「マッピング」で取得したアドレスにデータを書き込むには、`memcpy`や`std::copy`といった通常のコピー関数を使います。マッピングされたアドレスに書き込んだデータは、即座にGPUメモリへとコピーされます。このコピーは自動的に行われます。

ただし、GPUが「コピー先にあるデータ」を描画中の場合、そのデータを上書きしてしまうと描画結果がおかしくなってしまうことがあります。この問題を回避するには、CPUが書き込むバッファとGPUが参照する領域を分離します。この技法を「ダブルバッファ」といいます。
...
<p><code class="tnmai_code"><strong>【書式】</strong><br>
void glBindBufferRange(バッファの種類, 割り当て先のインデックス, SSBOの管理番号,<br>
&emsp;割り当て開始位置, 割り当てるサイズ);
</code></p>
...
OpenGL 4.6において定義されている「条件」は、`GL_SYNC_GPU_COMMANDS_COMPLETE`(ジーエル・シンク・ジーピーユー・コマンズ・コンプリート)だけで、選択の余地はありません。また、制御フラグは定義されていないため、常に`0`を指定します。
...
「制御フラグ」は`GL_SYNC_FLUSH_COMMANDS_BIT`(ジーエル・シンク・フラッシュ・コマンズ・ビット)を指定できます。ただ、このフラグはほとんど使われません。このフラグを指定すると、内部的に`glFlush`関数を呼び出しますが、それなら明示的に`glFlush`関数を呼び出すべきだからです。そのため、フラグには`0`を指定しています。
...
そこで、いくつか意味のある座標を配列として用意しておき、どの要素を使うかをランダムに決めることにします。スプライト構造体を定義するプログラムの下に、次のプログラムを追加してください。
...
+    makeTexcoord(0, 80, 32, 48),    // 中型機
+    makeTexcoord(0, 128, 32, 32),   // 小型機
+    makeTexcoord(128, 176, 48, 48), // 隕石(中)
+  };
...
>**【3章のまとめ】**
>
>* 表示するテクスチャ座標を指定するとき、ピクセル座標で設定できると分かりやすい。
