...
問題は、現在の`Engine`クラスには、外からカメラパラメータを設定する方法がないことです。そこで、カメラオブジェクトを取得する関数を追加します。`Engine.h`を開き、`Camera.h`をインクルードしてください。
...
次に、`standard_3D.vert`を開き、`matRotateScale`変数の定義を選択し、`Ctrl+X`で切り取ってください(エラーが出ますが、今は無視します)。
...
`standard_3D.vert`に戻り、シェーダのコンパイルが成功するように、カメラを回転させるプログラムを次のように変更してください。
...
再び`GameObject.cpp`を開いてください。そして、コンパイルが通るように、貼り付けたプログラムを次のように変更してください。
