...
<p align="center">
<img src="images/sc/1101-A.jpg" width="66%" /><br>
各装置の役割
</p>
...
現代では、主にデータベース処理などの事務計算に用いられる大型コンピューターです。サーバの親分のような存在で、一般に、大型～中型冷蔵庫くらいの大きさがあります。

メインフレームの最大の特徴は「高い信頼性」です。例えば、ハードウェアが多少故障しても、正常に動作を続けられるように設計されています。

また、エラーや操作ミスがあっても正しい状態を復元できるように、専用のOSやアプリケーションが使われています。

メインフレームは十個から数百個のCPUを搭載し、総メモリ容量は数百ギガバイト～数テラバイトに及びます。

現代のCPUやOSに使われている基本技術には、メインフレームで初めて採用され、進化してきたものが数多く含まれています(パソコンより歴史が古いので、当然といえば当然ですが)。
...
>IBM社のメインフレーム紹介ページ<br>
>`https://www.ibm.com/jp-ja/topics/mainframe`

#### (3) サーバ

サーバ(Server)は「給仕人」や「従者」という意味で、専門的な業務に用いられるコンピューターを指します。メインフレームと同じくらい大きなものからパソコンサイズのものまで、利用するOSやCPUによっていくつかの種類があります。

メインフレームほどではありませんが、パソコンなどとく比べると高い信頼性を持っています。数個から数十個のCPUと、数十ギガバイトから数百ギガバイトのメモリを搭載しています。

* UNIX(ユニックス)サーバ: UNIXというサーバ用OSを使用するサーバです。一般的に、高性能かつ大規模なコンピューターで構成されており、メインフレームの代用として使われることもあります。
* IA(アイエー)サーバ: パソコンと同じインテル系(インテル・アーキテクチャ)のCPUを使用するサーバです。そのため「PC(ピーシー)サーバ」とも呼ばれます。<br>OSにはLinux(リナックス)やWindowsが使われます。UNIXサーバより性能が低いかわりに安いので、現代では最も広く使われています。
...
* ブレード型サーバ: ラックマウント型をさらに小型化したサーバです。ラックマウント型では個別に搭載していた「電源」や「LANボード」などを別ユニットに分離し、複数のサーバで共有することで小型化を実現しています。<br>ラックマウント型以上にコンパクトかつ省電力ですが、ラックのような標準規格がないことから、価格は高めです。
...
パーソナル・コンピューターの略称で、個人が利用するコンピューターを指します。2022年現在では、1個のCPUと、数ギガバイトから数十ギガバイトのメモリを搭載する構成が一般的です。

一般に、サーバに比べると小型で、デスクトップ型、ノート型、タブレット型などさまざまな形状が存在します。
...
自動車や家電製品などを制御するためのコンピューターです。MCU(マイクロ・コントローラー・ユニット)とも呼ばれます。価格と消費電力を下げるためにCPUの性能は低く、メモリも数十キロバイトから数メガバイト程度しか搭載していません。
...
スーパーコンピューターの富岳になると、その性能は500PFLOPS(ペタ・フロップス)にもなります。これは、パソコン用CPUと比べて100万倍の性能です。
...
<p align="center">
<img src="images/sc/1401-1-A.jpg" width="36%" />
</p>

ダイの中身はいくつかの機能ブロックに分かれています。機能ブロックの配置や種類、数はCPUによって異なります。

<p align="center">
<img src="images/sc/1401-2-A.jpg" width="45%" />
</p>
...
| 制御ユニット | 制御装置 | デコーダが解読した命令に従って、他の装置に<br>指令を出す |
| キャッシュメモリ | 制御装置 | 主記憶装置から読み取ったデータを一時的に<br>記憶する |
...
コンピューターのさまざまな動作タイミングを合わせるために、「クロック・ジェネレータ」という回路が搭載されています。
...
クロックジェネレータはマザーボードに搭載されており、制御ユニットはマザーボードからクロック信号を受け取って、各ブロック、および他の装置とのタイミングを合わせます。

>**【マザーボード？】**<br>
>「マザーボード」はCPUを含めた各装置を接続するための基盤です。各装置を直接電線でつなぐのは、電線の数が多すぎてとても管理しきれません。<br>
>そこで、プラスチックの基盤を用意し、その表面(および内部)に配線します。そして、各装置はマザーボード上の接続端子に接続します。マザーボードを使うことで、簡単に装置を接続することができます。

<p align="center">
<img src="images/sc/1201-A.jpg" width="50%" /><br>
マザーボード
</p>
...
周波数5GHzのCPUの場合、1回の処理時間は0.2ナノ秒(=200ピコ秒)です。この時間の単位は記憶装置の速度をあらわすために使われます。2022年現在、CPUと記憶装置の動作速度と容量は次のようになります。
...
>「1次キャッシュメモリ」は長くて読みづらいため、L1(エルワン)キャッシュ、L2(エルツー)キャッシュ、L3(エルスリー)キャッシュと呼ばれることが多いです。<br>
>LはLevel(レベル)の頭文字です。

<p align="center">
<img src="images/sc/memory_hierarchy.png" width="50%" /><br>
コンピューターの記憶階層
</p>
...
CPUが記憶装置からデータを読み出すときは、最初にキャッシュメモリを調べます。データがキャッシュメモリにあれば、CPUはそのデータを使って演算を行います。データがキャッシュメモリになかった場合、仕方がないので主記憶装置からデータを読み出します。

このとき、必要なデータの前後の部分もまとめて読み出して、キャッシュメモリにコピーしておきます。これは、コンピューターが扱うデータは、配列のように連続して読み書きするものが多いからです。

CPUが読み書きするデータは、直前に読み書きしたデータのすぐ近くにある、という性質は「参照の局所性」と呼ばれています。

必要なデータがキャッシュメモリに記憶されている確率を「キャッシュヒット率」といいます。キャッシュメモリは、キャッシュヒット率が90%以上になるように設計されます。

>L3キャッシュだけが組み込まれている仮定すると、ヒット率が90%のときの平均的な読み書き速度は、<br>
>&emsp;100ナノ秒 × 0.1 + 10ナノ秒 × 0.9 = 19ナノ秒<br>
...
これらの基本動作は順番に実行され、1～5のすべてが完了した時点でひとつの命令の実行が完了します。基本動作ごとに独立した電子回路として作られているため、実行を終えた電子回路は、次のように即座に新しい命令の処理を始めます(①～⑧は異なる命令)。

<p align="center">
<img src="images/sc/cpu_pipeline.png" width="60%" />
</p>

このように、基本動作を並列に実行することで、最初の命令だけは5クロックかかりますが、それ以降の命令はあたかも1クロックごとに実行されるように見えます。基本動作を並列に実行するしくみを「パイプライン制御」といいます。
...
<p align="center">
<img src="images/sc/cpu_pipeline_misprediction.png" width="60%" /><br>
分岐予測に失敗した場合のパイプラインの動作
</p>

例えば、上の図の①が分岐命令だとします。分岐命令が「演算の実行」を完了するまでに、3つの命令②③④が読み出されます。しかし、これらは分岐の有無によって異なるはずです。

どちらの命令を読み込むかを決めるには、「分岐予測」という技術が使われています。とりあえず、今回は`if`の直後にある命令列を読み出すことにしましょう(「分岐しない」と予測)。

この命令列は「`if`の条件式が真」の場合に実行されるはずの命令です。基本動作4の結果、条件式が真ならば、命令③④⑤は何事もなく実行が続けられます。

しかし、条件式が偽の場合は、命令②③④はパイプラインから取り除かれます。その後、分岐先の命令を読み出し直します。命令②③④の読み出しに使った時間は無駄になりますが、仕方ありません。
...
>メインフレーム用などの高価なCPUでは、複数のパイプラインを用意して、分岐する場合としない場合の両方の命令を読み出してしまうものも存在します。しかし、非常に高価なため、一般のCPUで採用される気配はありません。
...
<p align="center">
<img src="images/sc/multi_processor.jpg" width="33%" /><br>
マルチプロセッサ
</p>
...
<p align="center">
<img src="images/sc/multi_core_processor.jpg" width="33%" /><br>
マルチコアプロセッサ
</p>
...
>* 各桁の大きさは10のべき乗で表現する。<br>1=10の0乗、10=10の1乗、100=10の2乗、0.1=10の-1乗など。
>* 基数は10。
...
>* 各桁の大きさ(桁の重み)は2のべき乗で表現する。<br>1=2の0乗、10=2の1乗、100=2の2乗、0.1=2の-1乗など。
>* 基数は2。
...
>* 各桁の大きさは8のべき乗で表現する。<br>1=8の0乗、10=8の1乗、100=8の2乗、0.1=8の-1乗など。
>* 基数は8。
...
>* 各桁の大きさは16のべき乗で表現する。<br>1=16の0乗、10=16の1乗、100=16の2乗、0.1=16の-1乗など。
>* 基数は16。
...
<p align="center">
<img src="images/sc/fixed_point_number.png" width="80%" />
</p>
...
| 10進数 | 2進数 | | 10進数 | 2進数 |
|:-------:|:---------:|---|:-------:|:---------:|
| +15.875 | 01111.111 | | -0.125  | 11111.111 |
|   ︙    |    ︙     | | -0.25   | 11111.110 |
| +0.875  | 00000.111 | | -0.375  | 11111.101 |
| +0.75   | 00000.110 | | -0.5    | 11111.100 |
| +0.625  | 00000.101 | | -0.625  | 11111.011 |
| +0.5    | 00000.100 | | -0.75   | 11111.010 |
| +0.375  | 00000.011 | | -0.875  | 11111.001 |
| +0.25   | 00000.010 | | -1      | 11111.000 |
| +0.125  | 00000.001 | |   ︙    |    ︙     |
|  0      | 00000.000 | | -16     | 10000.000 |
...
<p align="center">
<img src="images/sc/floating_point_number.png" width="80%" />
</p>

上の表では、仮数部の桁の重みを「?」としています。<br>
浮動小数点数では「仮数部の桁の重みは、指数部によって変化する」ためです。

さらに、浮動小数点数では、ビットを効率的に使うために、指数と仮数の表現方法を次のように定めています。
...
| 10進数 | 2進数 | | 10進数 | 2進数 |
|:------:|:-----------:|---|:------:|:-----------:|
|  非数   | 0 11111 11 | | -0      | 1 00000 00 |
|   ︙    |     ︙     | | -0.00000762939453125 | 1 00000 01 |
|  非数   | 0 11111 01 | | -0.00006103515625 | 1 00001 00 |
| +無限大 | 0 11111 00 | | -0.09375 | 1 01011 10 |
| +57344  | 0 11110 11 | | -0.109375 | 1 01011 11 |
| +49152  | 0 11110 10 | | -1      | 1 01111 00 |
|   ︙    |     ︙     | | -2      | 1 10000 00 |
| +2      | 0 10000 00 | |   ︙    |    ︙     |
| +1      | 0 01111 00 | | -49152  | 1 11110 10 |
| +0.109375 | 0 01011 11 | | -57344  | 1 11110 11 |
| +0.09375 | 0 01011 10 | | -無限大 | 1 11111 00 |
| +0.00006103515625 | 0 00001 00 | |  非数   | 1 11111 01 |
| +0.00000762939453125 | 0 00000 01 | |   ︙    |     ︙     |
| +0      | 0 00000 00 | |  非数   | 1 11111 11 |
...
<p align="center">
<img src="images/sc/Ssd_960.jpg" width="33%" /><br>
...
ほかにも、自然環境にある放射線の影響で、メモリの値が変化してしまうこともあります。
...
「パリティビット」とは、「誤り検出」および「誤り訂正」のために、元データに追加するビットデータのことです。
...
共通鍵暗号方式では「どうやって暗号鍵を相手に送るか」ということが問題となります。これを解決したのが「公開鍵暗号」方式です。

公開鍵暗号方式では、暗号化と復号に異なる鍵を使い、用途に合わせてどちらか片方の鍵だけを公開します。公開するほうを「公開鍵」、秘密にするほうを「秘密鍵」といいます。

最も一般的な利用方法は、「データの暗号化」と「送信者の認証」です。
...
署名によって送信者を特定できるようになりましたが、それだけではまだ問題があります。それは、赤の他人が「わたしはAです」といつわって、署名の復号鍵を公開する可能性があるからです(なりすまし)。
...
以前に出てきたデータを見つけたら、「現在位置から前に出てきたデータまでの距離」と「一致する長さ」で置き換えます。
...
「以前に出てきたデータを利用する」という性質から、ある程度以上の長さを持つデータに対して有効です。
