...
<pre class="tnmai_assignment">
<strong>【課題05】</strong>
次のURLから、<code>map01_bg.txt</code>という配置データファイルをダウンロードしなさい。
<code>https://github.com/tn-mai/OpenGL3D2022/blob/master/res/map01_bg.txt</code>
...
-    }
-  }
+  TileMap tileMap = engine.LoadTileMap("Res/map01_bg.txt");
...
<pre class="tnmai_assignment">
<strong>【課題06】</strong>
次のURLから、<code>map01_obj.txt</code>という背景データファイルをダウンロードしなさい。
<code>https://github.com/tn-mai/OpenGL3D2022/blob/master/res/map01_obj.txt</code>
ファイルをダウンロードしたら、プロジェクトの<code>Res</code>フォルダに保存しなさい。
</pre>

`map01_obj.txt`を開くと、ほとんどの値が`-1`で、時折`0`～`3`の数値が現れるデータになっています。数値の意味は次表のとおりです。
