...
次に、下部の「名前」欄を`Sprite.h`に変更します(②)。最後に「場所」として「プロジェクトの`Src`フォルダ」を選択したら(③)、右下にある「追加」ボタンをクリックしてください(④)。

これで、プロジェクトに`Sprite.h`というファイルが追加されます。追加した`Sprite.h`を開いたら、最初から書かれている`#pragma once`行を削除し、次のプログラムを追加してください。
...
`Main.cpp`を開き、`Shift`キーを押しながら矢印キー(またはマウスドラッグ)で`Texcoord`構造体と`Sprite`構造体の定義を選択し、`Ctrl+X`キーで切り取ってください。
...
行頭の空白が気になるので整形しましょう。上部メニューから「編集→詳細→ドキュメントのフォーマット」を選択してください。すると行頭の空白が消えます。

>コピペなどを行った結果、不要な空白ができてしまった場合などは、同じ手順で解消できます。
...
以後は`Sprite.h`をインクルードすることで、`std::vector<Sprite>`の代わりに`SpriteList`を使うことができます。

次に、テクスチャ座標を作成する`MakeTexcoord`を移動します。`Main.cpp`を開き、`MakeTexcoord`ラムダ式を切り取ってください。
...
`Sprite.h`を開き、`SpriteList`型の定義の下に、切り取ったプログラムを貼り付けてください。
...
再び`Main.cpp`を開き、次のように`Sprite.h`をインクルードしてください。`Texcoord`と`Sprite`の定義を移動させたことで発生したエラーが消えるはずです。
...
「全て`Run`関数に移動したし、これで完成！」…とはいきません。まだ何も「構造化」していないからです。そこで、プログラムの機能ごとに、以下の3つのパートに分けることにします。

>* 初期化
>* メインループ
>* 終了

プログラムを分割すると、あるパートで定義された変数を別のパートで使うことができなくなります。そのままではエラーになるので、パートをまたいで利用する変数を、クラスのメンバ変数にしておきます。
...
<pre class="tnmai_assignment">
<strong>【課題02】</strong>
<code>DebugCallback</code>と<code>CalcSsboSize</code>関数を、<code>Main.cpp</code>から<code>Engine.cpp</code>に移動しなさい。
</pre>
...
そして、`Engine`クラスの定義に次のプログラムを追加してください。
...
<p align="center">
<img src="images/04_result_0.png" width="45%" /><br>
</p>
...
| 定数 | 意味 |
|:-----|:-----|
| <ruby>GLFW_RELEASE<rt>ジーエルエフダブリュー・リリース</rt></ruby> | 押されていない |
| <ruby>GLFW_PRESS<rt>ジーエルエフダブリュー・プレス</rt></ruby> | 押されている |

### 2.7 自機をキー操作できるようにする

作成したばかりの`GetKey`関数を使って、戦闘機を移動できるようにしましょう。`MainGameScene.h`を開き、次のようにメンバ関数宣言を追加してください。
...
<p align="center">
<img src="images/04_result_1.png" width="45%" /><br>
</p>
...
<p align="center">
<img src="images/04_result_2.png" width="45%" /><br>
</p>
...
<p align="center">
<img src="images/04_result_3.png" width="45%" /><br>
</p>
...
そこで、画面外に飛び去った弾を削除します。弾を移動させるプログラムに、次のプログラムを追加してください。
...
+
+  // 弾を削除する条件をあらわすラムダ式
+  const auto isOutOfScreen = [tcBullet](const Sprite& e) {
+    if (e.texcoord.u == tcBullet.u && e.texcoord.v == tcBullet.v) {
+      return (e.x <= -32) || (e.x >= 18 * 32) ||
+        (e.y <= -32) || (e.y > 23 * 32);
+    }
+    return false;};
+
...
+    spriteListObj.begin(), spriteListObj.end(), isOutOfScreen);
+
+  // 配列の後ろ側を削除する
...
`vector`型配列から特定の要素を削除するには、`remove_if`(リムーブ・イフ)関数と`vector`型の`erase`(イレース)メンバ関数を使います。

<p><code class="tnmai_code"><strong>【書式】</strong><br>
iterator remove_if(調査範囲の先頭, 調査範囲の終端, 削除条件);
</code></p>

`remove_if`は、指定された範囲にある要素をすべて調べ、削除条件を「満たさない」要素を、範囲の前側に集めます。それ以外の削除してよい要素は後ろ側に移動されます。

`remove`は「削除」という意味ですが、名前に反して、実際には要素が削除されるとは限りません。実際に行われるのは、条件によって要素を前後に振り分けることだけです。

振り分けが終わると、後ろ側、つまり「削除してよい範囲」の先頭位置を返します。

「削除条件」はラムダ式などで指定します。今回は「画面外を判定する」ラムダ式なので、`isOutOfScreen`(イズ・アウト・オブ・スクリーン)という名前を付けました。

<p><code class="tnmai_code"><strong>【書式】</strong><br>
iterator erase(削除する範囲の先頭, 削除する範囲の終端);
</code></p>

`erase`(消去)関数は本当に要素を削除します。要素を削除するときは、上記のプログラムのように`remove_if`と`erase`を組み合わせて使います。

>**【remove_ifの通常版とメンバ関数版の違いについて】**<br>
>要素を削除しないのは、上記の「通常(非メンバ関数)の`remove_if`」だけという点に注意してください。標準ライブラリの`list`クラスや`unordered_map`クラスには「`remove_if`メンバ関数」が定義されていて、これら「メンバ関数の`remove_if`」は名前どおりに要素を削除します。
...
<pre class="tnmai_assignment">
<strong>【課題05】</strong>
次のURLから、<code>map01_bg.csv</code>という背景データファイルをダウンロードしなさい。
...
まず配置データをあらわす構造体を定義します。プロジェクトの`Src`フォルダに`TileMap.h`という名前のヘッダファイルを追加してください。追加したファイルを開き、次のプログラムを追加してください。
...
配置データファイルは普通のテキストファイルで、次のように数値がカンマで区切られています。
...
<p align="center">
<img src="images/04_result_5.png" width="45%" /><br>
</p>
...
+  spawnedY = enemyMap.sizeY; // 敵出現済みラインを初期化

   // 背景スプライトを作成(タイルサイズ=32x32)
   TileMap tileMap = engine.LoadTileMap("Res/map01_bg.csv");
```

配置データを読み込んだあと、出現済みラインをあらわす`spawnedY`(スポーンド・ワイ)メンバ変数を、最下段のY座標で初期化します。これは、敵の配置データと背景のスクロールを一致させるためです。
...
+
+  // 画面上部(天井)ラインの位置を計算
+  const int startY = static_cast<int>(enemyMap.sizeY) - 23;
+  int ceilY = startY + static_cast<int>(bgBaseY / 32);
+  if (ceilY < 0) {
+    ceilY = 0; // 範囲外アクセスを避けるために0を最小値とする
+  }
+
+  // 敵出現済みライン～天井ライン間の配置データに従って敵を出現させる
+  SpriteList& spriteListObj = engine.GetSpriteListObj();
+  for (; spawnedY > ceilY; --spawnedY) {
+    // 横一列分の配置データをチェック
...
<p align="center">
<img src="images/04_result_6.png" width="45%" /><br>
</p>
...
<p align="center">
<img src="images/04_result_7.png" width="45%" /><br>
</p>
