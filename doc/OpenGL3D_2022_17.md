...
>`Lon`は経度を意味する`longitude`(ロンギチュード)の短縮形、`Lat`は緯度を表す
>`Latitude`(ラチチュード)の短縮形です。
...
3D空間の衝突検出は2D空間よりも複雑です。しかし、全ての図形が複雑なわけではありません。最も簡単なのは、「球体」です。球体は「中心の座標」と「半径」の2つのパラメータで定義されます。
