...
>OpenGLにおいて、フレームバッファは`Framebuffer`という一単語で表現されます。`Frame`と`Buffer`の組み合わせではないことに注意してください。この違いが重要なのは「`Framebuffer Object`はバッファ・オブジェクトではない」からです(なお、英語としては`Frame Buffer`でも間違いではありません)。
...
続いて`Texture.cpp`を開き、`Create`メンバ関数の定義の下に、追加の`Create`メンバ関数を追加してください。
...
次に、コンストラクタの定義の下に、追加のコンストラクタを追加してください。
...
それでは、`FBO`を管理するクラスを作成しましょう。クラス名は`FramebufferObject`(フレームバッファ・オブジェクト)とします。
...
まずインクルードガードを書き、次に必要なヘッダファイルをインクルード、そして`FramebufferObject`クラスのポインタを先行宣言しています。

一般的なフレームバッファは「カラーバッファ」と「深度バッファ」の2つを持ちます。しかし、2Dスプライトのようにカラーバッファだけで深度バッファは不要だったり、エフェクトによっては逆に深度バッファだけで十分な場合もあります。

不要なバッファを作成するのはGPUメモリの無駄です。そこで、必要なバッファの種類を表す`FboType`(エフビーオー・タイプ)列挙型を定義し、この値を`FramebufferObject`コンストラクタに渡すことにします。
...
そこで、演算子オーバーロードによって、`FboType`専用の`&`演算子を定義することにします。`FboType`列挙型の定義の下に、次のプログラムを追加してください。
...
`FBO`は自身の管理番号に加えて、割り当てられたテクスチャを取得する関数を定義しています。

### 1.5 Createメンバ関数を定義する

続いてメンバ関数を定義します。プロジェクトの`Src`フォルダに`FramebufferObject.cpp`という名前のCPPファイルを追加してください。追加したファイルを開き、次のプログラムを追加してください。
...
1のケースでは「`texColor`が`nullptr`かどうか」を判定します。これは上記の`!texColor`の部分です。2のケースでは「管理番号が`0`かどうか」を判定します。これは上記の`!*texColor`の部分です。

常に適切なプログラムを書いている限りは、どちらのケースもまず起きないため、このエラーチェックはなくても問題にならないでしょう。しかし、「常に適切なプログラムを書く」ということは人類には不可能です。

そのため、早期にバグを発見できるように、できるだけ多くの場所にエラーチェックを入れておくことが推奨されます。とはいえ、チェックを入れ過ぎるとアプリの実行速度が低下します。バランスが重要です。

>**【!演算子の前後の空白について】**<br>
>なお、テキストで`!`演算子の前後に空白を入れているのは、見間違えを防ぐためです。一般的な書き方ではないので、実際に書き写すときは空白を入れる必要はありません。
...
検査の結果、`FBO`が正しく構築されていたら`GL_FRAMEBUFFER_COMPLETE`(ジーエル・フレームバッファ・コンプリート)という値が返されます。`FBO`に不良がある場合はそれ以外の値が返されます。これでコンストラクタは完成です。

### 1.7 デストラクタを定義する

続いてデストラクタを定義します。デストラクタでは`FBO`を破棄するだけです。テクスチャには`std::shared_ptr`を使っているため、自動的に破棄されます。コンストラクタの定義の下に、次のプログラムを追加してください。
...
それでは、ゲーム画面をFBOに描画してみましょう。まずゲームエンジンにFBOを追加します。`Engine.h`を開き、3Dモデル用の変数定義の下に、次のプログラムを追加してください。
...
描画先をデフォルトフレームバッファからゲームウィンドウ用FBOに変更するには、`glBindFramebuffer`(ジーエル・バインド・フレームバッファ)関数を使います。

`MainLoop`メンバ関数にある、スプライトデータをSSBOにコピーするプログラムの下に、次のプログラムを追加してください。
...
描画先をFBOに変更したため、ビューポートの設定も変更する必要があります。`MainGameScene.cpp`を開き、ビューポートを設定するプログラムを次のように変更してください。
...
+
+      const size_t size = spriteListGameWindow.size() * sizeof(Sprite);
+      memcpy(p, spriteListGameWindow.data(), size);
+      p += CalcSsboSize(size);
+    }
...
`blurStrength`(ブラー・ストレングス)変数は、ぼかしの強さを指定するメンバ変数です。`0`を指定すると全くぼけません。`1`を指定すると最大までぼかした状態になります。
...
影の描画に必要なのは深度バッファだけで、カラーバッファは不要です。これは、`standard_3D.vert`, `standard_3D.frag`で行っているほとんどの計算が不要であることを意味します。
...
フラグメントシェーダでは、テクスチャとマテリアルのアルファ値を参照し、アルファ値が`0.5`未満の場合は`discard`(ディスカード、「破棄する」という意味)キーワードによって描画をキャンセルします。
...
`look at`は「～を見る」という意味です。ワールド座標系から視点座標系への変換行列を返す関数名としてよく使われます。英語で書くと`The eye looks at
...
最初に軸の向きを計算します。カメラのZ軸は`target`から`eye`へ向かう方向なので、`eye - target`で求められます。さらに、向きは単位ベクトルでなくてはならないため、`normalize`関数によって正規化しています。
...
Y軸の計算では正規化を行っていないことに注意してください。X軸とZ軸は直角なので`θ=90°`、どちらも単位ベクトルなので長さは`1`です、これらを外積の定義に代入すると`|A||B|sinθ n = 1 * 1 * 1 * n = n`となるため、正規化は不要なのです。
...
のことです。また、この観点から三角関数を見ると、`cos`はX軸に対する射影、`sin`はY軸に対する射影と考えられます。あまり直感的ではありませんが、回転も射影の一種とみなすことができます。
...
垂直投影は「平行投影」の一種で、投影面に垂直な方向から光を当てた場合を指します。関数名は`Orthogonal`(オーソゴナル、「垂直」という意味)とします。`LookAt`関数の定義の下に、次のプログラムを追加してください。
...
`sampler2D`の場合、テクスチャから読み取った値をそのまま返します。それに対して`sampler2DShadow`は、テクスチャから読み取った値と引数で受け取ったZ座標を比較し、その結果を返します。
...
拡散反射と鏡面反射は光が届いているときしか起こりません。それに対して、環境光は全方位から届く光なので影の影響を受けません。そこで、拡散反射と鏡面反射の2つに対してだけ`shadow`変数を乗算しています。
...
影用FBOのサイズは影の品質に大きく影響するため、本音ではできるだけ大きくしたいところです。しかし、大きくするほど描画時間が長くなるためバランスが重要です。
...
テクスチャの幅は`GL_TEXTURE_WIDTH`(ジーエル・テクスチャ・ウィス)、高さは`GL_TEXTURE_HEIGHT`(ジーエル・テクスチャ・ハイト)で取得できます。
...
最後に、ゲームオブジェクトを描画し、影用ビュープロジェクション行列を返します。`matShadow`をユニフォーム変数にコピーするプログラムの下に、次のプログラムを追加してください。
...
<p align="center">
<img src="images/12_depth_map.png" width="33%" /><br>
[深度マップの例]
</p>
...
`Engine.cpp`を開き、`DrawStaticMesh`メンバ関数について、平行光源の定義を切り取り、`glUseProgram`の上に貼り付けてください。
...
そこで、今回はZ軸方向の描画範囲を100～2000とし、視点を-1000の位置に置くことにしました。これによってわーる座標系における表示範囲は-900～+1000となります。
...
```diff
   // 深度マップを作成
   const mat4 matShadow = CreateDepthMap(
     gameObjectList, shadowEye, shadowTarget, 1000, 10, 1000);
...
<p align="center">
<img src="images/12_shadow_acne.png" width="45%" /><br>
[黒=光が届かない 黄色=光が届く]
</p>
...
<p align="center">
<img src="images/12_shadow_acne_solution.png" width="45%" /><br>
[座標をずらすことで解像度の不一致を打ち消す]
</p>

座標を調整するにはいくつかの方法がありますが、今回は「座標を法線方向に少しだけ移動させる」方法を使います。`standard_3D.vert`を開き、影を計算するプログラムを次のように変更してください。
...
```diff
   ...
   // 影判定用の座標を計算
+  float worldShadowRange = 750 * 2; // 深度マップの描画範囲
...
プログラムが書けたらビルドして実行してください。不自然な暗さがなくなり、それらしい影が表示されていたら成功です。
