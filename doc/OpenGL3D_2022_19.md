...
最初の`if`文は、交差していない場合に距離計算をスキップして、処理を高速化するためのものです。交差していない場合は`0`(正の交差距離)を返します。
...
2つのOBBの角度によっては、外積によって得られたベクトルの長さが0になることがあります。この場合、2つの軸は平行であり、これはOBBの軸の判定で代用できるためスキップできます。
...
AABBを「回転していないOBB」として扱うことで、OBBとOBBの衝突判定を利用できます。
...
<p align="center">
<img src="images/19_intersect_sphere_capsule.png" width="25%" />
</p>
...
次に、カプセル用のコライダーを追加します。`Collider.h`を開き、`Type`列挙体の定義に次のプログラムを追加してください。
...
これで、球体とカプセルの衝突判定が実行されるようになりました。
...
とりあえず、球体同士の衝突として扱える場合の衝突判定から書いていきましょう。
...
+
+  // 射影した範囲が交差していない(分離超平面を入れる隙間がある)なら0を返す
+  if (minB > maxA || minA > maxB) {
+    return false;
+  }
+
+  // 衝突法線は「カプセル」が左、「OBB」が右にあると想定しているので、
+  // 順序が逆になっている場合、衝突法施の向きを反転する必要がある、
+  // ということを、呼び出し元に伝える
+  shouldFlip = minB < minA;
+
...
このプログラムは、1.4節のOBBとOBBの距離を計算するプログラムと同じものです。詳しい説明はそちらを参照してください。

これで、カプセルとOBBの交差範囲を求める`GetIntersectionLength`関数は完成です。
...
`pairList`配列は、分離軸のインデックスと射影に使われた頂点のインデックスから、目的の辺を作り出すための頂点を素早く選択するために使います。
...
>**【2章のまとめ】**
>
>* カプセルと球体、カプセルとカプセルの衝突判定は、カプセル中心にある線分の最近接点を求めることで計算できる。
>* 線分同士の最近接点を計算する方法は、連立方程式を立て、それを解くことで求められる。
>* カプセルとOBBの衝突判定は、「分離軸判定」と「球体とOBBの衝突判定」を組み合わせて行う。
>* コンタクトポイントを計算しなくてよい場合、衝突判定を簡素化できることが多い。
>* 複雑な図形同士の衝突判定は、さまざまな場合分けが必要となる。個々の計算は簡単だが、それらを適切に組み合わせる必要があるため、全体としてはかなり複雑になってしまう。
