...
+
+  // 2つのシェーダをリンクする
+  GLuint progStandard2D = glCreateProgram();
+  glAttachShader(progStandard2D, vs);
+  glAttachShader(progStandard2D, fs);
+  glLinkProgram(progStandard2D);
