...
1. Rendererクラスとその派生クラスStaticMeshRendererを作る。
2. StaticMeshRendererによるメッシュの描画(Engine::DrawStaticMesh)関数を実装する。
3. CameraにViewportパラメータを追加し、2D画面と同期するようにパラメータを調整する。
4. 自機が小さすぎるので、GameObjectにscaleメンバを追加し、シェーダに送って拡大縮小する。
5. すると自機がお尻を向けているので、X軸回転を追加する。
ここまでで1章の予定
6. さらにZも追加するとなると面倒なので、mat4を実装し、TRS行列を合成できるようにする。
mat4と添字演算子の追加、モデル行列による描画までが2章
ここまでで3D描画は完成。あと、どこかのタイミングでアンビエントライトを追加。
フレネル項を入れてもいいけど、そうするとPBRまでいかないといけないかもしれない。
ああ、ライト、ライトをどうしようか。個々の爆発(狭い範囲に大量に出現！)にライトを仕込むにはタイルベースドレンダリングが必要になるけど、これはかなり重いテキストになるから単独でやりたい。
UnityアセットからOBJを作る方法も必要だ。あと背景を3D化するにはちょっと頭をひねる必要があるか。いや、タイル化された3Dモデルをを使う手があるか。

### 1.1 

第09回までのプログラミングによって、OBJファイルとMTLファイルからメッシュを作り、割り当てられたマテリアルに応じた質感を表示できるようになりました。
...
「判定するクラス」には、`AddComponent`のテンプレート引数`T`を指定します。これによって、`T`が`MeshRenderer`かその派生クラスの場合のみ`meshRendererList`に追加されます。
...
>ここでは、通常のif文を使うこともできます。しかし、テンプレート引数がコンパイル時に確定することはC++規格で決まっているので、コンパイル時ifを使わない理由がありません。
...
描画関数の名前は`DrawStaticMesh`(ドロー・スタティック・メッシュ)とします。`AddComponent`メンバ関数の定義の下に、次のプログラムを追加してください。
...
+    const vec3 r = e->rotation + vec3(rotation.x, rotation.y, rotation.z);
...
次は、ゲームエンジンにメッシュを描画する機能を追加します。関数名は`DrawStaticMesh`(ドロー・スタティック・メッシュ)とします。`Engine.h`を開き、`Engine`クラスの定義に次のプログラムを追加してください。
...
次に、カーソルを`DrawSpriteList`関数の定義の下に移動し、切り取ったプログラムを`Ctrl+V`で貼り付けてください。
...
OBJファイルを読み込む関数の名前は、ストレートに`LoadOBJ`(ロード・オブジェ)とします。`Engine`クラスの定義に次のプログラムを追加してください。
...
そして、`ClearUILayers`メンバ関数の定義の下に、`LoadOBJ`関数の定義を追加してください。
...
OBJファイルを読み込む機能はすでに作ってあるので、この関数は単にそれ--`MeshBuffer::LoadOBJ`関数--を呼び出すだけです。

### 1.6 ゲームオブジェクトにメッシュレンダラを設定する

それでは、メッシュレンダラを使ってみましょう。`MainGameScene.cpp`を開き、`MeshRenderer.h`をインクルードしてください。
...
プログラムが書けたらビルドして実行してください。ゲームを開始するとメッシュが・・・表示されませんね？

### 1.7 カメラパラメータを設定する

ここでメッシュが表示されない理由は、「ゲームオブジェクトの座標がカメラの視野の外にあるから」です。カメラパラメータを変更すればすぐに表示されるはずです。ただ、現在は`Engine`クラス外でカメラパラメータを設定する方法がありません。

そこで、カメラオブジェクトを取得する関数を追加します。`Engine.h`を開き、`Engine`クラスの定義に次のプログラムを追加してください。
...
`shared_ptr`をキャストするには`static_pointer_cast`(スタティック・ポインタ・キャスト)関数テンプレートを使います。
...
>カメラコンポーネントは、カメラオブジェクトの0番目のコンポーネントと決め打ちしています。これはかなり危険な手段ですが、カメラオブジェクトのコンポーネントを削除したりしなければ問題ないはずです。

それでは、カメラのパラメータを調整していきます。まず前提条件として、ゲームオブジェクトの座標は2Dスプライト用になっています。現在のスプライトの描画範囲は`(17*32, 22*32)`です。

こういったゲーム全体で決まっている数値は、ゲームエンジンから取得できると便利でしょう。`Engine.h`を開き、`VecMath.h`をインクルードしてください。
...
さて、カメラは描画範囲全体を視野に入れる必要があります。しかしその前に、`VecMath`名前空間を利用可能にします。`MainGameScene.cpp`を開き、次のプログラムを追加してください。
...
とりあえず、カメラを+Z軸方向に500m動かして、-Z方向を向かせます。これでXY平面を正面から見ることになります。それから、描画範囲の中央を見るように位置を変更します。`Initialize`メンバ関数の定義に、次のプログラムを追加してください。
...
プログラムが書けたらビルドして実行してください。・・・まだ表示されませんね。あ、いえ、よく見ると時々なにかがちらついています。何かが表示されてはいるようです。

これは表示サイズが小さすぎるからです。そこで、メッシュの表示サイズを大きくしましょう。`MainGameScene.cpp`を開き、`Initialize`関数にある「自機を追加」するプログラムに、次のプログラムを追加してください。
...
続いて、`viewport`変数をビューポートに反映するプログラムを追加します。`Engine.cpp`を開き、`DrawStaticMesh`メンバ関数の定義に次のプログラムを追加してください。
...
そして、タンジェントから視野角に変換するには`atan`(エー・タン)関数を使います。
...
アスペクト比は`viewSize.y / viewSize.x`で求められます。それでは、カメラパラメータを設定するプログラムに、次のプログラムを追加してください。
...
このプログラムでは、`fovY`には度数法の値を設定する必要があります。しかし`atan`の戻り値は弧度法(ラジアン)なので、`VecMath.h`に定義した`degrees`関数を使って度数法に戻しています。
...
現在、メッシュの`rotation`メンバ変数は、Y軸回転にしか対応していません。しかし、自機を上から見下ろしたように表示するにはX軸で回転させる必要があります。そこで、X軸用のコサインとサインを追加します。

`standard_3D.vert`を開き、次のユニフォーム変数を追加してください。
...
Y軸回転ではXZ平面上で回転しましたが、X軸回転の場合はYZ平面上で回転します。
...
このような、「2つのベクトルの要素同士を乗算し、すべて加算する」という操作には

>内積(ないせき)

という名前が付いています。3次元ベクトルA, Bがあるとすると、内積は以下の式で表されます。

>`内積 = A.x * B.x + A.y * B.y + A.z * B.z`

GLSLでは内積は`dot`(ドット)関数として定義されています。内積を使って回転の計算を書き直してみましょう。`standard_3D.vert`を開き、X軸回転のプログラムを次のように変更してください。
...
同様に、ワールド法線の回転も内積で置き換えましょう。まず式をまとめます。ワールド法線を計算するプログラムを次のように変更してください。
...
次に式を内積で置き換えます。ワールド法線を計算するプログラムを、次のように変更してください。
...
内積を使うことでかなり式が簡単になりましたが、実はさらに簡単に書く方法があります。それが「行列(ぎょうれつ)」です。
...
ここで、`mat3x3`(マット・スリー・バイ・スリー)は「3列3行の行列」を意味します。`mat`は`matrix`(マトリックス)の短縮形です。`3x3`の最初の`3`は列数(配列のサイズ)、後ろの`3`が行数(ベクトルのサイズ)です。

>内積の第2引数の値が「縦に並んでいる」ことに注意してください。これは、OpenGLの慣習で縦に並べることになっているからです(横並びの行列も作れます)。

`mat3x3`は`float[3][3]`または`vec3[3]`と同じです。行数を減らした`mat2x3`は`float[2][3]`または`vec3[2]`と同じです。ベクトル配列の場合は、数字の並びが逆になる点に注意してください。
...
ベクトルと行列を乗算した結果はベクトルになります。注意したいのは、「ベクトル×行列」と「行列×ベクトル」では少し計算方法が異なる、という点です。
...
このように転置行列は右上と左下を入れ替えるだけで作れます。

ただ、逆行列と転置行列を組み合わせると、なぜ「拡大率の逆数を掛けた回転行列」が作れるのか、という問題は、ややこしい話になるので省略します。ここでは、

>1. 「拡大率の逆数を掛けた回転行列」を作るには、`inverse`(インバース)して`transpose`(トランスポーズ)する。この行列は「逆転置行列」という。
...
これで頂点シェーダのエラーは消えると思います。それでは、行列をユニフォーム変数にコピーしましょう。`GameObject.cpp`を開き、`DrawStaticMesh`メンバ関数の定義を次のように変更してください。
...
さて、`vec4 * mat4`の計算結果は`vec4`型になります。しかし、代入先の`outPosition`は`vec3`型です。そこで、今度は`vec3`コンストラクタによって`vec3`型に戻しています。
...
現在、ゲームオブジェクトが持つ回転パラメータのうちXとYだけが表示に影響します。せっかくなのでZ軸回転も加えたいのですが、すでに行列の初期化はじゅうぶん複雑になっており、Y軸回転とX軸回転をまとめたときのように簡単にはいきません。

そこで、軸ごとに回転行列を作り、行列をかけ合わせて全てをまとめることにします。そのために、平行移動、拡大縮小、X軸回転、Y軸回転、Z軸回転の5種類の行列を作成する関数を定義します。
...
行列とベクトルの乗算では、行列の各行とベクトルの各要素を乗算し、それをすべて加算するように定義します。
...
それでは、作成した関数を使って、モデル行列を作成するプログラムを変更しましょう。`GameObject.cpp`を開き、`DrawStaticMesh`メンバ関数の定義を次のように変更してください。
...
+
+  // 自機を左右に傾ける
+  player->rotation.z = vx * radians(-30.0f);
