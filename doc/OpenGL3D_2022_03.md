...
<pre class="tnmai_assignment">
<strong>【課題02】</strong>
<code>texcoordList</code>に要素を追加して、以下の画像が表示されるようにしなさい。
...
役割を明確にするため、既存のスプライト配列の名前を`spriteListObj`(スプライトリスト・オブジェ)に変更しました。背景用に追加したほうは`spriteListBg`(スプライトリスト・ビージー)という名前にしました。
